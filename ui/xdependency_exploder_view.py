...
class DependencyExploderView(tk.Toplevel):
    ...
    def populate_tree(self, node, parent=''):
        try:
            ...
            # --- NEW LOGIC START ---
            # Select which formula to display based on the checkbox
            if self.show_full_formula_var.get():
                formula = node.get('full_formula', node.get('formula', '')) # Use full, fallback to legacy
            else:
                formula = node.get('short_formula', node.get('formula', '')) # Use short, fallback to legacy

            # Do the same for the resolved formula
            resolved_formula = ""
            if (node.get('has_indirect', False) or node.get('has_index', False)
                or node.get('has_vlookup', False) or node.get('has_hlookup', False)):
                if self.show_full_formula_var.get():
            ...
            # Use the full formula for the details pane, regardless of display
