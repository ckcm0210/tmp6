...
class EnhancedDependencyExploder:
    ...
    def _create_short_formula(self, formula):
        """Creates a shortened version of a formula by simplifying external references."""
        if not formula:
            return formula
        
        # This regex finds a full path inside single quotes, e.g., 'C:\path\[file.xlsx]Sheet1'
        # and captures the part from the opening bracket to the end of the sheet name.
        # It correctly handles paths with spaces.
        pattern = re.compile(r"'[^']*\[(\[[^\]]+\][^']+)'")
        
        def replacer(match):
            # The captured group is the '[file.xlsx]Sheet1' part.
            # We return it enclosed in single quotes.
            return f"'{match.group(1)}'"
            
        try:
            # Replace all occurrences found by the pattern
            short_formula = pattern.sub(replacer, formula)
            return short_formula
        except Exception:
            # In case of any regex error, return the original formula
            return formula
    ...
    def _create_node_with_dynamic_functions(self, workbook_path, sheet_name, cell_address, current_depth, root_workbook_path, cell_info, fixed_formula, resolved_formula=None, indirect_info=None, index_info=None, vlookup_info=None, hlookup_info=None):
        ...
        node = {
            ...
            'formula': fixed_formula, # Legacy support
            'full_formula': fixed_formula,
            'short_formula': self._create_short_formula(fixed_formula),
        ...
        # --- NEW LOGIC FOR has_resolved ---
        node['has_resolved'] = (
            node.get('has_indirect', False) or
            node.get('has_index', False) or
            node.get('has_vlookup', False) or
            node.get('has_hlookup', False)
        )
        
        # 設置resolved_formula
        if has_dynamic_resolution and resolved_formula and resolved_formula != fixed_formula:
            node['resolved_formula'] = resolved_formula # Legacy support
            node['full_resolved_formula'] = resolved_formula
            node['short_resolved_formula'] = self._create_short_formula(resolved_formula)
                ...
                for ref in references:
                    if ref.get('is_range_summary'):
                        try:
                            ...
                            self.progress_callback.update_progress(f"範圍摘要節點建立失敗: {str(re_error)}")
