        ...
        var vis = (function() {
            ...
            Network.prototype.drawNode = function(ctx, node, x, y, isHighlighted) {
                ...
                lines.forEach((line, index) => {
                    ...
                    // + 新增：統一使用更簡潔的正規表示式移除所有HTML標籤，取得純文字。
                    var cleanLine = line.replace(/<[^>]*>/g, '');
                    ...
                    // +++ 以下是新的「純粹繪製邏輯」+++
                    // + 直接繪製已由 wrapText 處理好的一整行，不再做任何寬度判斷。
                    ctx.fillText(cleanLine, startX, currentY);
                    
                    // + 繪製完畢後，直接將Y座標下移一行的高度。
                    currentY += lineHeight;
                    // +++ 新邏輯結束 +++
                    
                    // + 修改了判斷條件，確保在兩行非空文字之間才增加額外間距，避免在結尾產生多餘空隙。
                    if (index < lines.length - 1 && cleanLine.trim() !== '' && lines[index+1].trim() !=='') {
                        currentY += 4;
                    }
                });
            };
            ...
            function updateNodeLabels() {{
                ...
                allNodes.forEach(function(node) {{
                    ...
                    if (formulaLabel && formulaLabel !== 'N/A' && formulaLabel !== null) {{
                        ...
                        var wrappedFormula = wrapText(displayFormula, lineWidth);
                    // 將換行後的每一行都獨立加上 <i> 標籤，確保所有行都是斜體
                    var italicizedLines = wrappedFormula.split('\\n').map(line => '<i>' + line + '</i>').join('\\n');
      
                    newLabel += '\\n\\nFormula : ' + italicizedLines;
                    }} else {{
                       newLabel += '\\n\\nFormula : <i>N/A</i>'; 
                    }}
